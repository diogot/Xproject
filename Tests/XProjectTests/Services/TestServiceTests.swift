...
// MARK: - Mock Build Service

actor MockBuildService: BuildServiceProtocol {
    struct BuildCall {
        let scheme: String
        let clean: Bool
    ...
    var buildForTestingCalls: [BuildCall] {
        _buildForTestingCalls
    }

    var runTestsCalls: [TestCall] {
        _runTestsCalls
    }

    func setShouldFailBuildForScheme(_ scheme: String, shouldFail: Bool) {
        shouldFailBuildForScheme[scheme] = shouldFail
    }

    func setShouldFailTestForDestination(_ destination: String, shouldFail: Bool) {
